...
option_list = list(
  make_option(c("-p", "--params"), type="integer", default=NULL,
...
# Run FlowSOM
labels <- repFlowSOM2(data, marker_cols = lineage_channels, nclust = nclust, seed = seed, n = size, n_stable = n_stable, reps = reps)
end_time <- Sys.time()
...
# Save result to file:
save(lineage_channels, params, labels, file=paste0('../results/02_flowsom2/flowsom2_', patient, '_', seed, '_',
                                                   size, '_', nclust, '.RData'))
